...
/* VibeXP icon set — inline Lucide-style strokes (the product uses lucide-react).
   24×24 viewBox, currentColor stroke, 2px default weight, round caps/joins.

   This is a curated subset for zero-dependency use (buildless prototyping, the
   docs site, common product glyphs). For the full icon set in production, use
   `lucide-react` directly — don't extend this map into a parallel icon library. */
const ICON_PATHS = {
...
export interface IconProps extends Omit<React.SVGProps<SVGSVGElement>, "name" | "stroke"> {
  /** Icon name from the built-in set (see ICON_NAMES). */
  name: IconName;
  /** Pixel size (width = height). @default 16 */
  size?: number;
  /** Stroke width (the SVG stroke colour is fixed to currentColor). @default 2 */
  stroke?: number;
  /** Accessible label. When set, the icon is exposed to AT; otherwise aria-hidden. */
  title?: string;
}

export function Icon({ name, size = 16, stroke = 2, className, style, title, ...rest }: IconProps) {
